...
class TestAsyncHelpers:
    ...
    async def test_analyze_flac_runs(self, mock_slskd_cls, mock_spotify):
        with patch("slskd_importer.telegram.download.delivery.analyze_flac") as mock_analyze:
    ...
    async def test_convert_to_ogg_runs(self, mock_slskd_cls, mock_spotify):
        with patch("slskd_importer.telegram.download.delivery.convert_to_ogg") as mock_conv:
    ...
    async def test_create_preview_runs(self, mock_slskd_cls, mock_spotify):
        with patch("slskd_importer.telegram.download.delivery.create_preview_clip") as mock_clip:
    ...
    async def test_embed_spotify_artwork_success(self, mock_slskd_cls, mock_spotify):
        ...
        with patch("slskd_importer.telegram.download.delivery.fetch_spotify_artwork") as mock_fetch:
            mock_fetch.return_value = b"\xff\xd8\xff\xe0"
            with patch("slskd_importer.telegram.download.delivery.embed_artwork_into_file") as mock_embed:
