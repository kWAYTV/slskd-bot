"""
Entry point for slskd-importer.
Can be run as: python -m slskd_importer
"""

import argparse
import logging
import sys

from slskd_importer import __version__
from slskd_importer.health.server import HealthHandler, start_health_server
from slskd_importer.settings import Config, setup_logging
from slskd_importer.telegram import create_bot

logger = logging.getLogger(__name__)

# Re-exported for tests and the CLI health endpoint.
_start_health_server = start_health_server

__all__ = ["HealthHandler", "cmd_run", "main"]


def cmd_run(args):
    """Run the Telegram bot with health check endpoint."""
    config = Config()
    setup_logging(config)

    logger.info("slskd-importer v%s starting", __version__)

    bot_app = create_bot(config)
    bot = bot_app.bot_data.get("music_bot")
    checker = bot.slskd.ping if bot is not None else None
    start_health_server(config.health_port, checker=checker)
    logger.info("Health check endpoint on 127.0.0.1:%s", config.health_port)

    logger.info("Starting Telegram bot polling")
    bot_app.run_polling(drop_pending_updates=True)


def main():
    """CLI entry point."""
    parser = argparse.ArgumentParser(
        prog="slskd-importer",
        description="Automated music discovery and download via Telegram bot.",
    )
    parser.add_argument("--version", action="version", version=f"%(prog)s {__version__}")

    subparsers = parser.add_subparsers(dest="command", help="Available commands")
    subparsers.add_parser("run", help="Start the bot and health server")

    args = parser.parse_args()
    command = args.command or "run"

    if command != "run":
        parser.print_help()
        sys.exit(1)

    cmd_run(args)


if __name__ == "__main__":
    main()
