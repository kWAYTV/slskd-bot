...
- **Pasted links**: `catalog/links.py` detects Spotify track links/URIs and SoundCloud track URLs in free text. Spotify tracks resolve via `SpotifyResolver.get_track`; SoundCloud via the public oEmbed endpoint (`catalog/soundcloud.py`, no API key — title format "Track by Artist"). Playlist/album links in plain text get a "use /import" hint
